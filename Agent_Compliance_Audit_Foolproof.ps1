...
function Generate-Dashboard {
    ...
    $TempJsonFile = Join-Path -Path $DashboardFolder -ChildPath "dashboard_data_$Timestamp.js"
    
    # Write the JSON data to the temporary file
    $JsonContent = "const dashboardData = " + ($DomainData | ConvertTo-Json -Depth 5 -Compress) + ";"
    $JsonContent | Out-File -FilePath $TempJsonFile -Encoding UTF8
    
    # Dashboard HTML template with external JSON reference
    $DashboardHtml = @"
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <!-- Load the data file -->
    <script src="dashboard_data_$Timestamp.js"></script>
