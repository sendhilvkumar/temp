...
const ContentGenerator = {
    ...
    generateOverviewContent: function() {
        ...
        if (!window.dashboardState || !window.dashboardState.consolidatedData) {
            console.warn('⚠️ No dashboard state or consolidated data available');
            return '<div class="info-box"><p>No data available. Please run data collection first.</p></div>';
        }
        
        const data = window.dashboardState.consolidatedData;
    ...
    generateUsersContent: function() {
        ...
        const domainData = this.getDomainData(window.dashboardState?.currentDomain || 'all');
    ...
    generateDomainsContent: function() {
        ...
        if (!window.dashboardState?.consolidatedData?.domainData) {
            return '<div class="info-box"><p>No domain data available.</p></div>';
        }
        
        const domains = Object.values(window.dashboardState.consolidatedData.domainData);
    ...
    generateReportsContent: function() {
        ...
        const metadata = window.dashboardState?.consolidatedData?.metadata || {};
        const summary = window.dashboardState?.consolidatedData?.aggregatedSummary || {};
    ...
    getDomainData: function(domainId) {
        if (!window.dashboardState?.consolidatedData) return null;
        
        if (domainId === 'all') {
            return {
                summary: window.dashboardState.consolidatedData.aggregatedSummary,
                metadata: window.dashboardState.consolidatedData.metadata
            };
        }
        
        return window.dashboardState.consolidatedData.domainData[domainId] || null;
    }
};
