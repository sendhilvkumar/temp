# Enterprise Compliance Dashboard Generator - Final Version with Charts & Graphs
# Scans CSV reports and generates static HTML dashboard with pure CSS/JS charts
# Version: 9.0
...
function Calculate-SummaryStats {
    ...
    foreach ($Domain in $AllDomainData) {
        ...
        if ($Domain.Data.ContainsKey("DomainControllers") -and $Domain.Data["DomainControllers"].Count -gt 0) {
            ...
            # Collect OS version data for charts
            foreach ($DC in $Domain.Data["DomainControllers"]) {
                $OSVersion = $DC.OSVersion
                if (-not [string]::IsNullOrWhiteSpace($OSVersion) -and $OSVersion -ne "N/A") {
                    if ($Stats.ChartData.OSVersions.ContainsKey($OSVersion)) {
                        $Stats.ChartData.OSVersions[$OSVersion]++
                    } else {
                        $Stats.ChartData.OSVersions[$OSVersion] = 1
                    }
                }
            }
        }
        ...
        # Count software packages and collect top software data
        if ($Domain.Data.ContainsKey("SoftwareInventory")) {
            $DomainStat.SoftwareCount = $Domain.Data["SoftwareInventory"].Count
            
            # Collect top software for charts
            foreach ($Software in $Domain.Data["SoftwareInventory"]) {
                $SoftwareName = $Software.DisplayName
                if (-not [string]::IsNullOrWhiteSpace($SoftwareName) -and $SoftwareName -ne "N/A") {
                    if ($Stats.ChartData.TopSoftware.ContainsKey($SoftwareName)) {
                        $Stats.ChartData.TopSoftware[$SoftwareName]++
                    } else {
                        $Stats.ChartData.TopSoftware[$SoftwareName] = 1
                    }
                }
            }
        }
        ...
        if ($Domain.Data.ContainsKey("CriticalAgents")) {
            ...
            # Collect agent status data for charts
            foreach ($Agent in $Domain.Data["CriticalAgents"]) {
                $Status = $Agent.Status
                if ($Status -eq "Installed" -or $Status -eq "Running" -or $Agent.HealthStatus -eq "Healthy") {
                    $Status = "Installed"
                } else {
                    $Status = "Not Installed"
                }
                
                if ($Stats.ChartData.AgentStatus.ContainsKey($Status)) {
                    $Stats.ChartData.AgentStatus[$Status]++
                } else {
                    $Stats.ChartData.AgentStatus[$Status] = 1
                }
            }
        ...
        if ($Domain.Data.ContainsKey("HardwareInfo")) {
            foreach ($Hardware in $Domain.Data["HardwareInfo"]) {
                $DiskUsage = $Hardware.DiskUsagePercent
                if (-not [string]::IsNullOrWhiteSpace($DiskUsage) -and $DiskUsage -ne "N/A") {
                    $DiskUsageNum = [double]::Parse($DiskUsage.Replace('%', ''))
                    if (-not $Stats.ChartData.HardwareStats.ContainsKey("DiskUsage")) {
                        $Stats.ChartData.HardwareStats["DiskUsage"] = @()
                    }
                    $Stats.ChartData.HardwareStats["DiskUsage"] += $DiskUsageNum
                }
            }
        }
...
# Function to generate chart data JavaScript
function Generate-ChartDataJS {
    param([hashtable]$SummaryStats)
    
    Write-Log "Generating chart data JavaScript"
    
    # OS Versions Chart Data
    $OSVersionsJS = "{"
    $OSVersionCount = 0
    foreach ($OSVersion in $SummaryStats.ChartData.OSVersions.Keys) {
        if ($OSVersionCount -gt 0) { $OSVersionsJS += "," }
        $OSVersionsJS += "`"$(Escape-HTML $OSVersion)`": $($SummaryStats.ChartData.OSVersions[$OSVersion])"
        $OSVersionCount++
    }
    $OSVersionsJS += "}"
    
    # Top Software Chart Data (top 10)
    $TopSoftwareJS = "{"
    $SortedSoftware = $SummaryStats.ChartData.TopSoftware.GetEnumerator() | Sort-Object Value -Descending | Select-Object -First 10
    $SoftwareCount = 0
    foreach ($Software in $SortedSoftware) {
        if ($SoftwareCount -gt 0) { $TopSoftwareJS += "," }
        $TopSoftwareJS += "`"$(Escape-HTML $Software.Key)`": $($Software.Value)"
        $SoftwareCount++
    }
    $TopSoftwareJS += "}"
    ...
    $AgentStatusJS = "{"
    $AgentCount = 0
    foreach ($Status in $SummaryStats.ChartData.AgentStatus.Keys) {
        if ($AgentCount -gt 0) { $AgentStatusJS += "," }
        $AgentStatusJS += "`"$(Escape-HTML $Status)`": $($SummaryStats.ChartData.AgentStatus[$Status])"
        $AgentCount++
    }
    $AgentStatusJS += "}"
    ...
    foreach ($DomainStat in $SummaryStats.DomainStats) {
        if ($DomainCount -gt 0) { $DomainComparisonJS += "," }
        $DomainComparisonJS += @"
{
    "domain": "$(Escape-HTML $DomainStat.DomainName)",
    "servers": $($DomainStat.ServerCount),
    "software": $($DomainStat.SoftwareCount),
    "patches": $($DomainStat.PatchCount),
    "compliance": $($DomainStat.ComplianceScore)
}
"@
        $DomainCount++
    }
    ...
    return @"
const chartData = {
    osVersions: $OSVersionsJS,
    topSoftware: $TopSoftwareJS,
    agentStatus: $AgentStatusJS,
    domainComparison: $DomainComparisonJS,
...
# Function to generate HTML dashboard with charts, search, sorting, CSV downloads, and easter egg
function Generate-HTMLDashboard {
    ...
    Write-Log "Generating HTML dashboard with charts, search, sorting, CSV downloads, and easter egg"
    ...
    $ChartDataJS = Generate-ChartDataJS -SummaryStats $SummaryStats
...
<body>
    ...
    <div class="charts-section">
        <h2 style="margin-bottom: 2rem; color: #0078d4; text-align: center;">📊 Data Visualization</h2>
        <div class="charts-grid">
            <!-- Compliance Gauge -->
            <div class="chart-container">
                <div class="chart-title">Overall Compliance Score</div>
                <div class="compliance-gauge">
                    <div class="gauge-circle" style="--percentage: $($SummaryStats.CompliancePercentage)">
                        <div class="gauge-inner">
                            <div class="gauge-value">$($SummaryStats.CompliancePercentage)%</div>
            ...
            <div class="chart-container">
                <div class="chart-title">Domain Comparison</div>
                <div class="bar-chart" id="domainComparisonChart">
            ...
            <!-- OS Versions Chart -->
            <div class="chart-container">
                <div class="chart-title">Domain Controller OS Versions</div>
                <div class="pie-chart" id="osVersionsChart">
                    <!-- Will be populated by JavaScript -->
                </div>
                <div class="pie-legend" id="osVersionsLegend">
                    <!-- Will be populated by JavaScript -->
                </div>
            </div>
            ...
            <!-- Agent Status Chart -->
            <div class="chart-container">
                <div class="chart-title">Critical Agents Status</div>
                <div class="donut-chart" id="agentStatusChart">
                    <!-- Will be populated by JavaScript -->
                </div>
                <div class="pie-legend" id="agentStatusLegend">
                    <!-- Will be populated by JavaScript -->
                </div>
            </div>
            ...
            <!-- Top Software Chart -->
            <div class="chart-container">
                <div class="chart-title">Top Software Packages</div>
                <div class="bar-chart" id="topSoftwareChart">
                    <!-- Will be populated by JavaScript -->
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function initializeCharts() {
            ...
            createOSVersionsChart();
            createAgentStatusChart();
            createTopSoftwareChart();
        }
        
        // Create domain comparison bar chart
        function createDomainComparisonChart() {
            const container = document.getElementById('domainComparisonChart');
            if (!container || !chartData.domainComparison) return;
            
            container.innerHTML = '';
            
            const maxServers = Math.max(...chartData.domainComparison.map(d => d.servers));
            
            chartData.domainComparison.forEach(domain => {
                const item = document.createElement('div');
                item.className = 'bar-item';
                
                const serverPercentage = maxServers > 0 ? (domain.servers / maxServers) * 100 : 0;
                
                item.innerHTML = `
                    <div class="bar-label">${domain.domain}</div>
                    <div class="bar-container">
                        <div class="bar-fill" style="width: ${serverPercentage}%">
                            <div class="bar-value">${domain.servers}</div>
                        </div>
                    </div>
                `;
        ...
        // Create OS versions pie chart
        function createOSVersionsChart() {
            const chartContainer = document.getElementById('osVersionsChart');
            const legendContainer = document.getElementById('osVersionsLegend');
            
            if (!chartContainer || !legendContainer || !chartData.osVersions) return;
            
            const data = chartData.osVersions;
            const total = Object.values(data).reduce((sum, val) => sum + val, 0);
            
            if (total === 0) {
                chartContainer.innerHTML = '<div style="text-align: center; padding: 2rem; color: #6c757d;">No OS version data available</div>';
                return;
            }
            
            const colors = ['#0078d4', '#106ebe', '#28a745', '#ffc107', '#dc3545', '#6f42c1', '#fd7e14', '#20c997'];
            let currentAngle = 0;
            let colorIndex = 0;
            ...
            Object.entries(data).forEach(([osVersion, count]) => {
                ...
                slice.style.background = `conic-gradient(${color} 0deg, ${color} ${angle}deg, transparent ${angle}deg)`;
        ...
        // Create agent status donut chart
        function createAgentStatusChart() {
            const chartContainer = document.getElementById('agentStatusChart');
            const legendContainer = document.getElementById('agentStatusLegend');
            
            if (!chartContainer || !legendContainer || !chartData.agentStatus) return;
            
            const data = chartData.agentStatus;
            const total = Object.values(data).reduce((sum, val) => sum + val, 0);
            
            if (total === 0) {
                chartContainer.innerHTML = '<div style="text-align: center; padding: 2rem; color: #6c757d;">No agent status data available</div>';
                return;
            }
            
            const colors = { 'Installed': '#28a745', 'Not Installed': '#dc3545' };
            let currentAngle = 0;
            ...
            center.innerHTML = `
                ...
                <div style="font-size: 0.8rem;">Total Agents</div>
            `;
            chartContainer.appendChild(center);
        }
        
        // Create top software bar chart
        function createTopSoftwareChart() {
            const container = document.getElementById('topSoftwareChart');
            if (!container || !chartData.topSoftware) return;
            
            container.innerHTML = '';
            
            const entries = Object.entries(chartData.topSoftware);
            if (entries.length === 0) {
                container.innerHTML = '<div style="text-align: center; padding: 2rem; color: #6c757d;">No software data available</div>';
                return;
            }
            
            const maxCount = Math.max(...entries.map(([, count]) => count));
            
            entries.slice(0, 8).forEach(([software, count]) => {
                const item = document.createElement('div');
                item.className = 'bar-item';
                
                const percentage = maxCount > 0 ? (count / maxCount) * 100 : 0;
                const shortName = software.length > 20 ? software.substring(0, 20) + '...' : software;
                
                item.innerHTML = `
                    <div class="bar-label" title="${software}">${shortName}</div>
                    <div class="bar-container">
...
"@
    ...
    try {
        ...
        Write-Log "HTML dashboard with charts generated: $OutputFilePath" -Level "SUCCESS"
...
# Main execution
try {
    Write-Log "=== Enterprise Compliance Dashboard Generator (Final with Charts & Graphs) Started ===" -Level "SUCCESS"
    ...
    Write-Log "Charts included: Compliance Gauge, Domain Comparison, OS Versions, Agent Status, Top Software" -Level "SUCCESS"
